...
async function getAirQuality({ location, range = 'current' } = {}) {
  ...
  if (range === 'hourly') {
    ...
    const hourly = data.airHourlyForecast || data.hourly || [];
    const next6 = hourly.slice(0, 6);
    const lines = next6.map(h => {
      const cn = (h.indexes || []).find(i => i.code === 'cn_aqi') || (h.indexes || [])[0];
  ...
  if (range === 'daily') {
    ...
    const daily = data.airDailyForecast || data.daily || [];
    const lines = daily.slice(0, 5).map(d => {
      const cn = (d.indexes || []).find(i => i.code === 'cn_aqi') || (d.indexes || [])[0];
      return cn ? `${d.forecastDate || ''} AQI ${cn.aqiDisplay} ${cn.category}` : '';
    }).filter(Boolean);
    return {
      isError: false,
      content: [{ type: 'text', text: `${cityLabel} 未来几天空气质量：\n${lines.join('\n')}` }],
      structuredContent: { city: loc.name, adm2: loc.adm2, range: 'daily', daily },
      handoff: () => ({
        query: `cityId=${loc.id || ''}&lon=${loc.lon}&lat=${loc.lat}`,
        payload: { city: { id: loc.id, name: loc.name, adm2: loc.adm2, lon: loc.lon, lat: loc.lat } }
      })
