...
### 图表增强
- [x] **#10** 30 天降水概率图：weather30 页补降水概率柱状图，数据已在接口返回
- [x] **#11** 生活指数等级可视化：进度条 / 圆点替代纯文字等级

### 其他
- [x] **#7** 桌面快捷入口引导：`wx.addShortcut` 或操作指引 modal
- [x] **#12** 城市时区 / 时差提示：收藏城市列表展示与本地时差，来自 GeoAPI `utcOffset` 字段
