...
Component({
  ...
  methods: {
    ...
    _drawChart() {
      ...
      // 云量固定 0-100 范围
      const valToY = (v) => padT + (1 - v / 100) * chartH;
      ...
      const points = hourly.map((d, i) => ({
        x: padL + i * stepX,
        y: valToY(Number(d.cloud) || 0)
      }));

      // 渐变填充（灰蓝色调）
      const gradient = ctx.createLinearGradient(0, padT, 0, padT + chartH);
      gradient.addColorStop(0, 'rgba(176,190,197,0.35)');
      gradient.addColorStop(1, 'rgba(176,190,197,0)');
      ctx.beginPath();
      ...
      points.forEach((p, i) => {
        const val = Number(hourly[i].cloud) || 0;
        if (i === sel) {
